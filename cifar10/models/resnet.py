...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.linear1 = nn.Linear(512*block.expansion, 128)# added by han
        self.linear2 = nn.Linear(128, num_classes)# added by han
    ...
    def forward(self, x):
        ...
        out = out.view(out.size(0), -1)
        out = self.linear1(out)# added by han
        out = self.linear2(out)# added by han
        return out

# ImageNet models
def ResNet18(input_size=32, num_class=10):
    return ResNet(BasicBlock, [2,2,2,2], num_class)

def ResNet18_noshort(input_size=32, num_class=10):
   return ResNet(BasicBlock_noshortcut, [2,2,2,2], num_class)

def ResNet34(input_size=32, num_class=10):
    return ResNet(BasicBlock, [3,4,6,3], num_class)

def ResNet34_noshort(input_size=32, num_class=10):
   return ResNet(BasicBlock_noshortcut, [3,4,6,3], num_class)

def ResNet50(input_size=32, num_class=10):
    return ResNet(Bottleneck, [3,4,6,3], num_class)

def ResNet50_noshort(input_size=32, num_class=10):
   return ResNet(Bottleneck_noshortcut, [3,4,6,3], num_class)

def ResNet101(input_size=32, num_class=10):
    return ResNet(Bottleneck, [3,4,23,3], num_class)

#def ResNet101_noshort(num_class):
#    return ResNet(Bottleneck_noshortcut, [3,4,23,3], num_class)

def ResNet152(input_size, num_class):
    return ResNet(Bottleneck, [3,8,36,3], num_class)

#def ResNet152_noshort(num_class):
#    return ResNet(Bottleneck_noshortcut, [3,8,36,3], num_class)
